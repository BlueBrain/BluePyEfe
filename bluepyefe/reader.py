...
def axon_reader(in_data):
    ...
    for trace in bl.segments:
        ...
        if np_trace.shape[0] == 2:
            ...
            raise Exception(
                f"Unknown .abf format for file {fp}. Maybe " "it does not have current data?"
            )

        data.append(
            {"voltage": numpy.asarray(v).flatten(), "current": numpy.asarray(c).flatten(), "dt": dt}
        )
...
def igor_reader(in_data):
    ...
    _check_metadata(in_data, igor_reader.__name__, ["v_file", "i_file", "t_unit"])
...
def _format_nwb_trace(voltage, current, start_time, trace_name=None, repetition=None):
    v_array = numpy.array(voltage[()] * voltage.attrs["conversion"], dtype="float32")

    i_array = numpy.array(current[()] * current.attrs["conversion"], dtype="float32")

    dt = 1.0 / float(start_time.attrs["rate"])
...
def _nwb_reader_BBP(content, target_protocols, repetition):
    ...
    for ecode in target_protocols:
        for cell_id in content["data_organization"].keys():
            ...
            rep_iter = _get_repetition_keys_nwb(ecode_content, request_repetitions=repetition)
            ...
            for rep in rep_iter:
                for sweep in ecode_content[rep].keys():
                    for trace_name in list(ecode_content[rep][sweep].keys()):
                        ...
                        if key_current not in content["stimulus"]["presentation"]:
                            logger.debug(
                                f"Ignoring {key_current} not"
                                " present in the stimulus presentation"
                            )
                            continue
                        if trace_name not in content["acquisition"]:
                            logger.debug(f"Ignoring {trace_name} not" " present in the acquisition")
                            continue
                        ...
                        data.append(
                            _format_nwb_trace(
                                ...
                                current=content["stimulus"]["presentation"][key_current]["data"],
                                start_time=content["stimulus"]["presentation"][key_current][
                                    "starting_time"
                                ],
