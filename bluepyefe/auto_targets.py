...
class AutoTarget:
    ...
    def select_ecode_and_amplitude(self, recordings):
        ...
        for ecode in self.protocols:
            ...
            for amp in self.amplitudes:
                n_available_rec = sum(abs(a - amp) < self.tolerance for a in available_amps)
                if n_available_rec >= self.min_recordings_per_amplitude:
                    self.active_ecode.append(ecode)
                    self.active_amplitudes.append(amp)

        if self.active_ecode and len(self.active_ecode) > self.preferred_number_protocols:
            self.active_ecode = self.active_ecode[: self.preferred_number_protocols]
            self.active_amplitudes = self.active_amplitudes[: self.preferred_number_protocols]
    ...
    def generate_targets(self):
        ...
        for amp, protocol_name in zip(self.active_amplitudes, self.active_ecode):
