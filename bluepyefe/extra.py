...
def spikerate_tau_jj(peaktimes):
    ...
    # example:
    #times = numpy.array([-0.99, -0.945, -0.874, -0.859, -0.64, -0.573, -0.433, -0.042, -0.007, 0.054, 0.088, 0.222, 0.401, 0.465, 0.633, 0.637, 0.735, 0.762, 0.791, 0.981])
    #freq = numpy.array([0.418, 0.412, 0.452, 0.48, 0.453, 0.501, 0.619, 0.9, 0.911, 0.966, 0.966, 1.123, 1.414, 1.683, 2.101, 1.94, 2.473, 2.276, 2.352, 3.544])

    n = len(times)
    S = [0]
    for k in range(1, n):
        S.append(S[k-1] + 1/2.*(freq[k]+freq[k-1])*(times[k]-times[k-1]))
    S = numpy.array(S)
...
def spikerate_tau_fit(peaktimes, nstart=1, nend=2):
    ...
    #drop = spikerate_drop(peaktimes, nstart=nstart, nend=nend)
